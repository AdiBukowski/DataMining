...
Krzywe ROC
<<>>=
pred.ROCR.spam <- prediction(lr.spam$fitted.values, y)
perf.ROCR.logit <- performance(pred.ROCR.spam, "tpr", "fpr")
plot(perf.ROCR.logit, print.cutoffs.at=seq(0.1,1,0.1), colorize=TRUE, lwd=2)
pred.ROCR.spam.sig <- prediction(lr.spam.sig$fitted.values, y)
perf.ROCR.logit <- performance(pred.ROCR.spam.sig, "tpr", "fpr")
plot(perf.ROCR.logit, print.cutoffs.at=seq(0.1,1,0.1), colorize=TRUE, lwd=2)

AUC.spam <- performance(pred.ROCR.spam, "auc")@y.values[1]
AUC.spam.sig <- performance(pred.ROCR.spam.sig, "auc")@y.values[1]
@
